...
//    case Straight:
    ...
    {
        // if(parameterinfo->ZUpdate != 0)
        // ROS_INFO("%f", (float)parameterinfo->parameters.Period_T/parameterinfo->parameters.Sample_Time);
...
void WalkingTrajectory::inversekinmaticsinfo()
{
    ...
    // parameterinfo->ik_parameters.R_Goal[1] = parameterinfo->points.IK_Point_RY - 4.5;
    ...
    // parameterinfo->ik_parameters.L_Goal[1] = parameterinfo->points.IK_Point_LY + 4.5;
...
void WalkingTrajectory::SaveData()
{
    ...
    for(int i = 0; i < test.size(); i++)
    {
        savedText = DtoS(osc_move_x_r[i]) + "\t"
                ...
                + DtoS(left_Thta[i]) + "\t"
                + DtoS(test[i]) + "\n";
        // Wave.R_move_X=osc_move_x_r[i];
        // Wave.L_move_X=osc_move_x_l[i];
        // Wave.R_move_Y=osc_move_y_r[i];
        // Wave.L_move_Y=osc_move_y_l[i];
        // Wave.R_move_Z=osc_move_z_r[i];
        // Wave.L_move_Z=osc_move_z_l[i];
        // Wave.move_COM_X=osc_move_com_x[i];
        // Wave.move_COM_Y=osc_move_com_y[i];
        // Wave.move_COM_Z=osc_move_com_z[i];
        // Wave.R_Thta=right_Thta[i];
        // Wave.L_Thta=left_Thta[i];
        // Wave.Point=test[i];
        // Wavelist.wavelist.push_back(Wave);
        //ROS_INFO("Wavelist = %f",Wavelist.wavelist[i].R_move_X);
        fp<<savedText;
    }
    //WaveValue_Publish.publish(Wavelist);
    //Wavelist.wavelist.clear();
    osc_move_x_r.clear();
    osc_move_x_l.clear();
    osc_move_y_r.clear();
...
double WalkingTrajectory::OSC_lifemove_DOWNz(double range, double period_T_ms, double rho_z, double delta_z, double life_high, int time_t_ms, int Sample_Time)
{
    ...
    {
        ROS_INFO("1");
        parameterinfo->complan.lift_lock_z = 0;
        return 0;
    } 
    else if(time_t >= range * T_divby4 && time_t < T_divby2 - range * T_divby4)
    {
        ROS_INFO("2");
        time_t -= period_T/Sample_Time*parameterinfo->complan.lift_lock_z;
        outputZ = rho_z * sin(omega_z * (time_t - range * T_divby4));
        ROS_WARN("%f", time_t);
        if (outputZ >= abs(life_high))
        {
            parameterinfo->complan.lift_lock_z++;
    ...
    else if(time_t >= T_divby2 + range * T_divby4 - period_T/Sample_Time*3&& time_t < T_divby2 + T_divby4 )
    {
        ROS_INFO("4");
        time_t -= period_T/Sample_Time*parameterinfo->complan.lift_lock_z;
        outputZ = rho_z * sin(omega_z * (time_t - range * T_divby4));
...
/*void WalkingTrajectory::initRosNodeHandle(ros::NodeHandle &n)
{
