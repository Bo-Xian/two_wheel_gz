#include "imageprocess/main/imagemain.h"
...
int main(int argc, char** argv)
{
    ...
    string ImageName;
    nh.getParam("ImageName", ImageName);
    nh_private.deleteParam("ImageName");
    if(ImageName == "kidsize_sim")
    {
        //ROS_INFO("1");
        Gazeboimage_subscriber = nh.subscribe("/kidsize/camera/image_raw", 1, Catch_image);
    }
    else if(ImageName == "twowheeler_sim")
    {
        //ROS_INFO("2");
        Gazeboimage_subscriber = nh.subscribe("/twowheeler/camera/image_raw", 1, Catch_image);
    }
    else if(ImageName == "kidsize")
    {
        Imagesource_subscriber = nh.subscribe("/usb_cam/image_raw", 10, GetImagesourceFunction);
    }
    //ROS_INFO("%s", DataUnit->ImageName.c_str());
